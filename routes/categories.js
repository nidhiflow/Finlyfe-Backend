...
router.post('/', async (req, res) => {
  ...
  try {
    const result = await query(
      ...
      [id, req.userId, name, type, icon, color, parent_id || null]
    );
...
router.put('/:id', async (req, res) => {
  ...
  try {
    const result = await query(
      ...
      [name, type, icon, color, parent_id || null, id, req.userId]
    );
...
router.delete('/:id', async (req, res) => {
  const { id } = req.params;
  try {
    const result = await query('DELETE FROM categories WHERE id = $1 AND user_id = $2 RETURNING *', [id, req.userId]);
    if (result.rows.length === 0) return res.status(404).json({ error: "Category not found" });
