...
const authenticateToken = (req, res, next) => {
    ...
    jwt.verify(token, JWT_SECRET, (err, decoded) => {
        ...
        req.userId = decoded.userId || decoded.id; // Handle both payload versions
        req.user = decoded;
        next();
    });
};
...
router.post('/resend-otp', async (req, res) => {
    try {
        ...
        // Delete old OTPs
        await query('DELETE FROM otp_codes WHERE email = $1 AND type = $2', [email, type || 'signup']);
...
router.post('/forgot-password', async (req, res) => {
    try {
        ...
        await query("DELETE FROM otp_codes WHERE email = $1 AND type = 'reset'", [email]);
